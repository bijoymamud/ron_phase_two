      ...
      <div className="bg-white md:shadow-md rounded-lg md:p-6 p-2 w-full max-w-4xl space-y-6 ">
        ...
        >
          ...
          <label className="block text-sm font-medium text-gray-700">
            Did you have any specific injuries during your service?
            <select
              {...register("hadInjuries", {
                required: "This field is required",
              })}
              className={`mt-1 block w-full p-2 dark:bg-white dark:border-black dark:text-black border uppercase border-gray-300 rounded-md  text-sm ${errors.hadInjuries ? "border-red-500" : ""}`}
            >
          ...
          {hadInjuries === "yes" && (
            <>
              ...
              <label className="block text-sm font-medium text-gray-700">
                Please list the injuries, how they occurred, and where they
                occurred (what base/location)
                <textarea
                  {...register("injuryDetails", {
                    required: "This field is required",
                  })}
                  className={`mt-1 block w-full p-2 dark:bg-white dark:border-black dark:text-black border border-gray-300 uppercase rounded-md  text-sm h-32 resize-none ${errors.injuryDetails ? "border-red-500" : ""}`}
                  placeholder="Enter details"
                />
                {errors.injuryDetails && (
                  <span className="text-red-500 text-sm">
                    {errors.injuryDetails.message}
                  </span>
                )}
              </label>
            </>
