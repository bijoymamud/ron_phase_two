...
//                   type="checkbox"
//                   {...register(item.key)}
//                   className="w-4 h-4 mt-0.5 text-[#0B2A52] rounded "
//                 />
...
const STORAGE_KEY = "ptsdOthersTemp";

const PTSDOthers = () => {
  const { register, control, watch } = useForm({
    defaultValues: {
      // EXACT CHECKBOX KEYS FROM BACKEND (untouched)
      increased_decreased_visits_to_a_healthcare_professional_counselor_or_treatment_facility_checkbox: false,
      request_for_a_change_in_occupational_series_or_duty_assignment_checkbox: false,
      behavioral_changes_experienced_following_the_traumatic_event_1: false,
      behavioral_changes_experienced_following_the_traumatic_event_2: false,
      ...
      // EXACT TEXTAREA KEYS FROM BACKEND (untouched)
      increased_decreased_visits_to_a_healthcare_professional_counselor_or_treatment_facility:
        "",
      request_for_a_change_in_occupational_series_or_duty_assignment: "",
      ...
      changes_in_eating_habits_such_as_overeating_or_undereating_or_significant_changes_in_weight:
        "",
      pregnancy_tests_around_the_time_of_the_traumatic_event: "",
  ...
  // Real-time save – works perfectly with your existing data
  useEffect(() => {
    const subscription = watch((value) => {
  ...
  const items = [
    ...
    {
      checkbox: "pregnancy_tests_arround_the_time_of_the_traumatic_event",
      details: "pregnancy_tests_around_the_time_of_the_traumatic_event",
      label: "Pregnancy tests around the time of the traumatic event(s)",
    },
    {
      checkbox: "tests_for_sexually_transmitted_infections",
      details: "testss_for_sexually_transmitted_infections",
      label: "Tests for sexually transmitted infections",
    },
    {
      checkbox: "economic_or_social_behavioral_changes",
      details: "economics_or_social_behavioral_changes",
      label: "Economic or social behavioral changes",
    },
    {
      checkbox: "changes_in_or_breakup_of_a_significant_relationship",
      details: "changes_in_or_breakup_of_a_significant_relationships",
      label: "Changes in or breakup of a significant relationship",
    },
  ];
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8">
      <h2 className="text-2xl font-bold text-[#0B2A52] mb-3">
        OTHER INDICATORS OF BEHAVIORAL CHANGES FOLLOWING TRAUMATIC EVENT(S)
      </h2>
      <p className="text-gray-600 mb-8">
      ...
      <div className="space-y-5">
        {items.map((item) => {
          const isChecked = !!formValues?.[item.checkbox];

          return (
            <div
              key={item.checkbox}
              className={`border-2 rounded-xl transition-all duration-200 ${
                isChecked ? "border-[#0B2A52] bg-blue-50" : "border-gray-300"
              }`}
            >
              <label className="flex items-start p-5 cursor-pointer select-none">
                <input
                  type="checkbox"
                  {...register(item.checkbox)}
                  className="w-5 h-5 text-[#0B2A52] rounded focus:ring-2 focus:ring-[#0B2A52]"
                />
                <span className="ml-4 text-gray-800 font-medium leading-relaxed">
                  {item.label}
                </span>
              </label>
              ...
              {isChecked && (
                <div className="px-5 pb-5">
                  <textarea
                    ...
                    rows={4}
                    className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#0B2A52] focus:border-[#0B2A52] outline-none resize-none text-gray-700"
                    placeholder="Provide details: When? How often? Any records?"
                  />
                </div>
              )}
            </div>
